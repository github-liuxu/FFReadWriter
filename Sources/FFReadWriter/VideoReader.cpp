...
AVFrame* VideoReader::ReadAVFrame() {
    AVFrame *frame = NULL;
    while (true) {
    ...
    int64_t time = av_rescale_q(frame->pts, time_base, AV_TIME_BASE_Q);
    if (time >= m_end) {
        av_frame_unref(frame);
        av_frame_free(&frame);
        return nullptr;
...
LXVideoFrame* VideoReader::ReadVideoFrame() {
    AVFrame *frame = NULL;
    while (true) {
        AVPacket *packet = av_packet_alloc();
        int ret = av_read_frame(formatContext, packet);
    ...
    LXVideoFrame *videoFrame = GetVideoFrame(frame);
    videoFrame->SetPts(time);
    return videoFrame;
}
...
bool VideoReader::Seek(int64_t timestamp) {
    AVRational time_base = formatContext->streams[videoStreamIndex]->time_base;
    int64_t time = av_rescale_q(timestamp, AV_TIME_BASE_Q, time_base);
